# Prismatica: Architecture Design Specification v0.5.3 (Production Ready)

**Goal:** Establish a robust architecture that integrates Prismatica's specific gameplay logic (from v0.2) with the provided "Core" infrastructure modules, while preserving the "Juice" implementation details (from v0.3) and addressing UI/Input gaps.
...
*   **Presentation (The Face):** Handles *feedback* (Shaders, VFX, SFX, UI).
...
*   **Signals:** `sfx_play_requested`, `music_play_requested`, `critical_error_occurred`, `toast_notification_requested`.
...
*   **Connections:**
    ...
    *   Listens to `GameCore` (via signal) for Ability Cooldowns -> Updates `OverloadProgressBar`.
    *   Listens to `CoreEventBus.toast_notification_requested` -> Spawns transient info popups (e.g. "Game Saved").

---

## 6. Input & Interaction (Consolidated)
...
*   **Interaction Model (Defined):** **Click-to-Select / Click-to-Place.**
    *   *Reasoning:* Preferred for Accessibility and future Mobile porting (vs Drag-and-Drop).
    *   *Flow:* Player clicks a "Square" icon in HUD (State = Placing). Player clicks a valid grid cell.
