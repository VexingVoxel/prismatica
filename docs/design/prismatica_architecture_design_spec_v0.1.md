# Prismatica: POC Architecture & Design Specification v0.1
...
* **Goal:** Establish the "Flow" (Low Cognitive Load) and "Visceral" (High Engagement) axes immediately.
...
* **The Hook (Tinkerer):** **Adjacency Bonuses.** "Squares generate +10% if touching the Core." Sets up the expectation of a spatial puzzle.
...
2.  **The Grid Data Resource:**
    * **Structure:** A `Dictionary` or custom `Resource`.
    * **Format:** `{ Vector2(0,0): { "type": "Square", "level": 10 } }`.
    * **Why:** Allows fast calculation of Adjacency Bonuses without iterating through Godot Scene Tree nodes (which is slow and messy).

3.  **The Visual Layer (Nodes):**
    * **Structure:** A `Node2D` container.
    * **Logic:** Listens to the Core/Grid signals and updates visuals only.
    * **Optimization:** Does *not* calculate game logic.
